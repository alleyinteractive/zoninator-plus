...
class Zoninator_Plus_Groups {
	...
	function add_group_zones( $id, $zones, $replace = false ) {
		...
		foreach( $zones as $zone ) {
			$result = fm_add_term_meta( $zone, z_get_zoninator()->zone_taxonomy, $this->zone_group_meta_key, $id );
			if( !$result )
				return new WP_Error( 'zone-group-add-zone', __( 'There was an error adding zones to the group.', 'zoninator-plus' ) );

			$result = fm_add_term_meta( $zone, z_get_zoninator()->zone_taxonomy, $this->zone_group_order_meta_key, $zone_order );
			if( !$result )
				return new WP_Error( 'zone-group-add-zone', __( 'There was an error adding zones to the group.', 'zoninator-plus' ) );
	...
	function remove_group_zones( $id, $zones=array() ) {
		...
		// Add all zones to the group
		foreach( $zones as $zone ) {
			$result = fm_delete_term_meta( $zone, z_get_zoninator()->zone_taxonomy, $this->zone_group_meta_key );
			if( !$result )
				return new WP_Error( 'zone-group-delete-zone', __( 'There was an error removing zones from the group.', 'zoninator-plus' ) );

			$result = fm_delete_term_meta( $zone, z_get_zoninator()->zone_taxonomy, $this->zone_group_order_meta_key );
			if( !$result )
				return new WP_Error( 'zone-group-delete-zone', __( 'There was an error removing zones from the group.', 'zoninator-plus' ) );
		}
	 ...
	 *
	 * @param bool $exclude_grouped optional
	 * @return array
	 */
	function get_zones( $include_group = false ) {
		...
		if( $zones && $include_group ) {
			...
			foreach( $zones as $zone ) {
				$zone_group = fm_get_term_meta( $zone->term_id, z_get_zoninator()->zone_taxonomy, $this->zone_group_meta_key, true );
				$zone_group_order = fm_get_term_meta( $zone->term_id, z_get_zoninator()->zone_taxonomy, $this->zone_group_order_meta_key, true );
				$zones_with_groups[] = array(
					'zone' => $zone,
	...
	function save_zone_fields( $term_id, $tt_id, $taxonomy ) {
		...
		if( $taxonomy == z_get_zoninator()->zone_taxonomy ) {
			...
			if( $zone_group != "" ) {
				...
				fm_update_term_meta( $term_id, z_get_zoninator()->zone_taxonomy, $this->zone_group_meta_key, $zone_group );

				// See if the zone group order is set
				// If not, set it after the last current zone in the group
				$zone_order = fm_get_term_meta( $term_id, z_get_zoninator()->zone_taxonomy, $this->zone_group_order_meta_key, true );
				if( $zone_order == "" ) {
					$group_zones = $this->get_group_zones( $term_id );
					$zone_order = count( $group_zones ) + 1;
					fm_update_term_meta( $term_id, z_get_zoninator()->zone_taxonomy, $this->zone_group_order_meta_key, $zone_order );
				}
			} else {
				// The zone group is not set
				fm_delete_term_meta( $term_id, z_get_zoninator()->zone_taxonomy, $this->zone_group_meta_key );
				fm_delete_term_meta( $term_id, z_get_zoninator()->zone_taxonomy, $this->zone_group_order_meta_key );
			}
		...
		if( $zoninator->is_zoninator_page() && !$this->is_zoninator_group_page() ) {
			...
			if( in_array( $zoninator->zone_taxonomy, $taxonomies ) && $zone_group_selected !== false ) {
				...
				foreach( $terms as $term ) {
					// Get the zone group
					$zone_group = fm_get_term_meta( $term->term_id, $zoninator->zone_taxonomy, $this->zone_group_meta_key, true );
